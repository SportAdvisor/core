...
package object http extends FailFastCirceSupport {
  ...
  object Validator {
    def apply[T](rules: (T => Option[ValidationResult])*): Validator[T] = new DefaultValidator[T](rules)
  }

  def validatorDirective[T](model: T, validator: Validator[T], i18nService: I18nService): Directive1[T] = {
    selectLanguage().tflatMap(t =>
      validator(model) match {
        case Nil => provide(model)
        case errors: List[ValidationResult] => reject(ValidationError(errors.map(_.toFormError(i18nService.errors(t._1)))))
      }
    )
  }
  ...
  def selectLanguage(): Directive1[String] = {
    extractRequest.map { request ⇒
      ...
      val pickLanguage = negotiator.pickLanguage(List(Language("ru"), Language("en"))) getOrElse Language("en")
      negotiator.acceptedLanguageRanges.find(l => l.matches(pickLanguage)).map(l => l.primaryTag)
          .getOrElse("en")
    }
  }
